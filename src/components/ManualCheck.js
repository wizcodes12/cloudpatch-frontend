...
const handleAutoFix = async (error, solution) => {
  ...
  try {
    ...
    // Instead of putting all data in the URL, use a POST request to a new endpoint
    const createIssueResponse = await fetch(`${API_BASE_URL}/create-github-issue`, {
      method: 'POST',
      headers: {
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json",
    ...
    if (!createIssueResponse.ok) {
      throw new Error("Failed to initialize GitHub issue creation");
    }
    
    const issueCreationData = await createIssueResponse.json();
    
    // Open the issue creator in a new window, but with minimal data in URL
    const width = 1000;
    const height = 800;
    ...
    const popupWindow = window.open(
      issueCreationData.redirect_url,
    ...
    const messageHandler = function(event) {
      // Same event handling code as before
